...
Prepare a labeled dataset of support tickets (minimum 2,000–3,000 examples for robust training). The dataset should include:
- Raw ticket text (subject and body)
- Ground truth category labels (5 classes)
- Ground truth priority labels (4 classes)
- Optional: Knowledge base articles or FAQ content for the RAG component

Once ready, open `model_training2.py` in a notebook environment (Kaggle, Colab, or local Jupyter) and:

1. Load your dataset into the notebook
2. Import the training utilities from `backend/src/`
3. Run all cells to train SetFit models, calibrate confidence scores, and build retrieval indices
4. Download the `results/` folder (containing trained models, calibration transforms, and precomputed embeddings) and place it at `backend/results/`

### 2. Recalibrate (if needed)

If classification confidence scores drift after retraining or data distribution shifts, recalibrate the models:

```python
import joblib

# Load and retrain calibration transforms if needed
for path in ["backend/results/models/calibration_category.pkl",
    ...
    # Update calibration parameters based on fresh validation set
    # Retrain on a held-out validation set to tune the calibration model
    joblib.dump(pkl, path)
```

See the `Confidence Calibration` section for details on how the system ensures reliable confidence scores.
